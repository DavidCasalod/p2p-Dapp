  ...
  **/
   async  function startCec() {
    ...
    try {
       ...
       const transientData = JSON.parse(json1);
       const transientString = JSON.stringify(transientData);
          
      
        const string1 = JSON.stringify(json);
        console.log('string: ' + transientString);
        let ctx = { contractedByEmail: "ross.little@atos.net", cecTradingParams: string1 }
        await contract.createTransaction("startCecContract")
        .setTransient(transientString)
        .submit("619", "contractedByByOrgI", "contractStart", "contractEnd", "state", "algorithm", "version");
